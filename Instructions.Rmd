...
```{R eval=F}
# Both datasets are already tidy, so I'll demonstrate the use of pivot_wider() on part of my data.
untidy_data1 <- data1 %>% pivot_wider(names_from="Regional indicator", values_from="Country name")
glimpse(untidy_data1)

tidy_data1 <- untidy_data1 %>% pivot_longer(c("Western Europe":"South Asia"), names_to="Country name", values_to="Regional indicator")
glimpse(tidy_data1)
```
...
joined_data <- inner_join(data1, data2, by=c("Country name"="Country"))
glimpse(joined_data)
```
